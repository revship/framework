...
class Revship_Database_Driver_Mysqli extends Revship_Database_Abstract
{
...
/*
    ...
    public function connect($dbConfig = null)
    {
        if(!$this->connected) {
            ...
            // Use prefered server first.
            //if(!$this->usePreferedServer($dbConfig)) new Revship_Exception(501,'Unable to parse master database config.');
            
            $time_start = $this->microtime_float();
            // Connect
            $this->connection = mysqli_connect($this->host, $this->userName, $this->passwd, $this->database, $this->port);
           
            // Use secondary server if failed
            if(false === $this->connection) 
            {
                //if($this->useSecondaryServer($dbConfig))
                 //   $this->connection = mysqli_connect($this->host, $this->userName, $this->passwd, $this->database, $this->port);
            }
            // If still failed. Try to connect prefered server 3 times
            if(false === $this->connection) 
            {
                //$this->usePreferedServer($dbConfig);
                for($i=0; $i<3; $i++) 
                {
                    $this->connection = mysqli_connect($this->host, $this->userName, $this->passwd, $this->database, $this->port);
                    if($this->connection) {
                ...
                throw new Revship_Exception(502,'Failed to connect database. Please check configuration or enhance database servers. ');
                return false;
            }
        ...
        {
            $this->connected = true;
            mysqli_query($this->connection, "set names ".$this->encoding);
        }
        else
        {
            throw new Revship_Exception(501,$this->lastError());
        }
        return $this->connected;
    }
    ...
    public function getAll($sql=null)
    {
        ...
        {    
            ...
            Revship::log("SQL Error: {$sql}",'ERROR');
    ...
    public function select($sSelect, $unsetAll)
    {
        if($unsetAll)
        {
                $this->unsetAll();
    ...
    public function execute($sql=null)
    {
        ...
        {
            Revship::log('SQL Trace: '.$sql);
            return mysqli_affected_rows($this->connection);
        }
        else
        {
             Revship::log(__CLASS__.'::'.__METHOD__."-SQL Error: {$sql}",'ERROR');
            if(Revship::lib('config')->getItem('site.debug')==true)
            {
                new Revship_Exception(501,"Execute Error [{$sql}]".$this->lastError());
            }
            else
            {
                new Revship_Exception(500,"DB Query Error");
            }
        }
    }
     ...
     */
    public function insert($table, $valueArray = array(), $escape = true, $returnQuery = false)
    { 
        ...
        $sql = $this->_insert($table, implode(', ', array_keys($valueArray)), $valueString);
    ...
    public function delete($table, $query, $limit = null)
    {
        ...
        return $this->query("DELETE FROM " . $this->table_prefix .$table. " WHERE ". $query);        
    }   
    ...
    {
        return 'INSERT INTO '  . $this->table_prefix . $table . ' '.
            '        (' . $sFields . ')'.
            ' VALUES (' . $sValues . ')';            
    }
