...
class Revship_Breadcrumbs
{
    ...
    public function generate($links){
        $this->links = $links;
        $output = '';
        $output .= Revship::lib('html')->openTag($this->tagName, $this->htmlOptions) . "\n";
        $links = array ();
        if ($this->homeLink === null)
        {
            $links [] = Revship::lib('html')->link(Revship::l('Home'), '/');
        }
        ...
        foreach ( $this->links as $label => $url ) 
        {
            ...
            {
                $links [] = Revship::lib('html')->link($this->encodeLabel ? Revship::l(Revship::lib('html')->encode($label)) : $label, $url);
            }
            else
            {
                $links [] = '<span>' . ($this->encodeLabel ? Revship::l(Revship::lib('html')->encode($url)) : $url) . '</span>';
            }
        }
        $output .= implode($this->separator, $links);
        $output .= Revship::lib('html')->closeTag($this->tagName);
        return $output;
    }
}
